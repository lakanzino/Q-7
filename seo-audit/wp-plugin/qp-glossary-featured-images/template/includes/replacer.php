...
function qpfi_process_row( $row, $dry_run = false ) {
	...
	if ( $current['id'] && '' !== $current['file'] && $current['basename'] === $row['file'] ) {
		...
		if ( ! empty( $options['set_alt'] ) ) {
			...
			update_post_meta( $current['id'], '_qpfi_alt_source', 'qp-fi-' . QPFI_VERSION );
		}
	...
	if ( ! empty( $options['set_alt'] ) ) {
		...
		update_post_meta( $new_id, '_qpfi_alt_source', 'qp-fi-' . QPFI_VERSION );
	}
 ...
 * @return array
 */
function qpfi_run_batch( $offset, $step = 0 ) {
	$map   = array_values( qpfi_map() );
	$total = count( $map );

	// اندازهٔ مرحله خودکار تنظیم می‌شود (هاست کند ← مرحلهٔ کوچک‌تر).
	if ( $step < 1 ) {
		$step = (int) get_option( 'qpfi_batch_size', QPFI_BATCH_SIZE );
	}
	...
	for ( $i = 0; $i < $step; $i++ ) {
		...
		$rows[] = qpfi_process_row( $map[ $index ], false );
	...
	update_option( 'qpfi_batch_size', $new_step, false );

	// از مرحلهٔ اول، گزارش قبلی پاک و از نو ساخته می‌شود.
	$stored = ( 0 === (int) $offset ) ? array() : get_option( QPFI_LOG, array() );

	if ( ! is_array( $stored ) ) {
		$stored = array();
	}

	update_option( QPFI_LOG, array_merge( $stored, $rows ), false );

	if ( $done ) {
		qpfi_clear_caches();
		delete_option( 'qpfi_batch_offset' );
	} else {
		update_option( 'qpfi_batch_offset', $next, false );
	}
