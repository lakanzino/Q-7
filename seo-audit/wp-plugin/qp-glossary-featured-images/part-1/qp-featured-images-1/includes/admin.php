 ...
 * @return void
 */
function qpfa_admin_menu() {
	add_submenu_page(
		'tools.php',
		qpfa_part_label(),
		sprintf( 'تصاویر شاخص جدید (بستهٔ %s)', number_format_i18n( QPFA_PART ) ),
		'manage_options',
		'qpfa',
		'qpfa_render_page'
	);
}
add_action( 'admin_menu', 'qpfa_admin_menu' );
 ...
 * @return array
 */
function qpfa_preview_row( $row ) {
	$post    = qpfa_find_article( $row['slug'] );
	$current = $post ? qpfa_current_image( $post->ID ) : array( 'id' => 0, 'basename' => '', 'url' => '', 'file' => '' );

	$others = ( $current['id'] ) ? qpfa_attachment_used_elsewhere( $current['id'], $post ? $post->ID : 0 ) : array();
...
function qpfa_render_page() {
	...
	if ( ! empty( $_POST['qpfa_action'] ) ) {
		if ( ! isset( $_POST['qpfa_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['qpfa_nonce'] ) ), 'qpfa_run' ) ) {
			$notices[] = array( 'error', 'نشانهٔ امنیتی معتبر نبود.' );
		} else {
			$action = sanitize_key( $_POST['qpfa_action'] );
			...
			if ( 'options' === $action ) {
				...
				delete_option( 'qpfa_batch_offset' );
				...
				$offset = isset( $_POST['qpfa_offset'] ) ? max( 0, (int) $_POST['qpfa_offset'] ) : 0;
				$batch  = qpfa_run_batch( $offset );
				$fresh  = 0;
				...
				if ( $batch['done'] ) {
					$report = get_option( QPFA_LOG, array() );
					$done   = 0;
				...
				if ( 'selected' === $action && ! empty( $_POST['qpfa_selected'] ) ) {
					$only = array_map( 'sanitize_title', explode( ',', (string) wp_unslash( $_POST['qpfa_selected'] ) ) );
				}
	...
	} elseif ( isset( $_GET['qpfa_report'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'qpfa_report' ) ) {
		$stored = get_option( QPFA_LOG, array() );
		$report = is_array( $stored ) ? $stored : array();
	}

	// اگر اجرای مرحله‌ای نیمه‌کاره مانده باشد، دکمهٔ «ادامه» نشان داده می‌شود.
	if ( null === $progress ) {
		$saved_offset = (int) get_option( 'qpfa_batch_offset', 0 );

		if ( $saved_offset > 0 && $saved_offset < count( qpfa_map() ) ) {
			$progress = $saved_offset;
		}
	}

	$map     = qpfa_map();
	$options = qpfa_options();
	...
	<div class="wrap" dir="rtl">
		<h1><?php echo esc_html( qpfa_part_label() ); ?></h1>

		<?php foreach ( $notices as $notice ) : ?>
			<div class="notice notice-<?php echo esc_attr( $notice[0] ); ?>"><p><?php echo esc_html( $notice[1] ); ?></p></div>
		<?php endforeach; ?>

		<?php if ( ! qpfa_other_part_active() ) : ?>
			<div class="notice notice-warning">
				<p><strong>بستهٔ دیگر فعال نیست.</strong> این افزونه «بستهٔ <?php echo esc_html( number_format_i18n( QPFA_PART ) ); ?> از <?php echo esc_html( number_format_i18n( QPFA_PARTS_TOTAL ) ); ?>» است و <strong><?php echo esc_html( number_format_i18n( count( $map ) ) ); ?> مقاله</strong> را پوشش می‌دهد. برای کامل شدن همهٔ <?php echo esc_html( number_format_i18n( QPFA_ARTICLES_TOTAL ) ); ?> مقاله، افزونهٔ بستهٔ دیگر را هم نصب و فعال کنید.</p>
			</div>
		<?php endif; ?>

		<?php if ( null !== $progress ) : ?>
			<?php
			$qpfa_total = count( $map );
			$qpfa_pct   = $qpfa_total ? (int) round( $progress / $qpfa_total * 100 ) : 0;
			?>
			<div class="notice notice-info" style="border-inline-start-color:#2271b1;padding:12px 14px">
				<p style="margin:0 0 8px"><strong>اجرای مرحله‌ای در جریان است — <?php echo esc_html( number_format_i18n( $progress ) ); ?> از <?php echo esc_html( number_format_i18n( $qpfa_total ) ); ?> مقاله (<?php echo esc_html( number_format_i18n( $qpfa_pct ) ); ?>٪)</strong></p>
				<div style="height:12px;background:#e0e0e0;border-radius:6px;overflow:hidden;max-width:620px"><div style="height:12px;width:<?php echo esc_attr( $qpfa_pct ); ?>%;background:#2271b1"></div></div>
				<form method="post" id="qpfa-continue" style="margin:10px 0 0">
					<?php wp_nonce_field( 'qpfa_run', 'qpfa_nonce' ); ?>
					<input type="hidden" name="qpfa_action" value="batch" />
					<input type="hidden" name="qpfa_offset" value="<?php echo esc_attr( $progress ); ?>" />
					<button type="submit" class="button button-primary">ادامه (حدود <?php echo esc_html( number_format_i18n( (int) get_option( 'qpfa_batch_size', QPFA_BATCH_SIZE ) ) ); ?> مقاله)</button>
					<button type="submit" class="button" name="qpfa_action" value="batch_cancel" formnovalidate>توقف</button>
					<span style="color:#646970;margin-inline-start:8px">صفحه خودش چند لحظه دیگر ادامه می‌دهد…</span>
				</form>
			</div>
			<script>
			(function(){
				var form = document.getElementById('qpfa-continue');
				if (!form) { return; }
		...
		<form method="post">
			<?php wp_nonce_field( 'qpfa_run', 'qpfa_nonce' ); ?>
			<input type="hidden" name="qpfa_action" value="options" />
			<p>
		...
		<?php if ( $show_preview ) : ?>
		<form method="post" id="qpfa-form">
			<?php wp_nonce_field( 'qpfa_run', 'qpfa_nonce' ); ?>
			<input type="hidden" name="qpfa_selected" id="qpfa-selected" value="" />

			<p>
				<button type="submit" name="qpfa_action" value="batch" class="button button-primary" onclick="return confirm('<?php echo esc_js( sprintf( 'اجرای مرحله‌ای: در هر مرحله %s مقاله تا پایان %s مقالهٔ این بسته؛ تصویر قبلی هم جایگزین یا حذف می‌شود. شروع شود؟', number_format_i18n( QPFA_BATCH_SIZE ), number_format_i18n( count( $map ) ) ) ); ?>');">شروع اجرا (مرحله‌ای و ایمن — پیشنهادی)</button>
				<button type="submit" name="qpfa_action" value="check" class="button">فقط بررسی (بدون تغییر)</button>
				<button type="submit" name="qpfa_action" value="selected" class="button">اجرای ردیف‌های تیک‌خورده</button>
				<button type="submit" name="qpfa_action" value="run" class="button" onclick="return confirm('<?php echo esc_js( sprintf( 'همهٔ %s مقالهٔ این بسته در یک درخواست اجرا شود؟ روی هاست کند ممکن است مرورگر قطع کند.', number_format_i18n( count( $map ) ) ) ); ?>');">اجرای همه در یک درخواست</button>
			</p>

			<table class="widefat striped">
				<thead>
					<tr>
						<th style="width:32px"><input type="checkbox" id="qpfa-all" /></th>
						...
						<tr>
							<td>
								<?php if ( 'missing' !== $state[0] ) : ?>
									<input type="checkbox" class="qpfa-row" value="<?php echo esc_attr( $row['slug'] ); ?>" />
								<?php endif; ?>
							</td>
							<td>
								<?php if ( $p['current']['id'] ) : ?>
							...
							<td>
								<img src="<?php echo esc_url( QPFA_URL . 'includes/images/' . $row['file'] ); ?>" alt="" loading="lazy" style="width:100px;height:56px;object-fit:cover;border-radius:4px" />
		...
		<p style="margin-top:14px">
			...
			<a class="button" href="<?php echo esc_url( wp_nonce_url( qpfa_page_url( array( 'qpfa_report' => 1 ) ), 'qpfa_report' ) ); ?>">نمایش گزارش ذخیره‌شده</a>
		</p>

		<h2 class="title">راهنمای کوتاه</h2>
		<ul style="list-style:disc;padding-inline-start:22px;line-height:2">
			<li><strong>پیشنهادی — «شروع اجرا (مرحله‌ای)»:</strong> در هر مرحله فقط <?php echo esc_html( number_format_i18n( QPFA_BATCH_SIZE ) ); ?> مقاله پردازش می‌شود و صفحه خودکار تا پایان ادامه می‌دهد؛ به‌این‌ترتیب نه نصب سنگین است و نه مرورگر/سرور قطع می‌کند. اگر وسط کار قطع شد، همین صفحه دکمهٔ «ادامه» را نشان می‌دهد و از همان‌جا ادامه می‌یابد.</li>
			...
			<li><strong>دو بسته:</strong> تصاویر به دو افزونهٔ سبک تقسیم شده‌اند تا نصب سریع باشد. این صفحه «بستهٔ <?php echo esc_html( number_format_i18n( QPFA_PART ) ); ?> از <?php echo esc_html( number_format_i18n( QPFA_PARTS_TOTAL ) ); ?>» است؛ بعد از اجرای این بسته، بستهٔ دیگر را نصب و فعال کنید و اجرای مرحله‌ای را برای آن هم بزنید.</li>
		</ul>
	</div>

	<script>
	(function(){
		var all = document.getElementById('qpfa-all');
		var rows = Array.prototype.slice.call(document.querySelectorAll('.qpfa-row'));
		var form = document.getElementById('qpfa-form');
		...
		if (form) {
			form.addEventListener('submit', function(event){
				...
				document.getElementById('qpfa-selected').value = picked.join(',');
			});
		}
	})();
	</script>
