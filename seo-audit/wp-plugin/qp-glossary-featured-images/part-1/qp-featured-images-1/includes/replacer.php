 ...
 * @return WP_Post|null
 */
function qpfa_find_article( $slug ) {
 ...
 * @return array
 */
function qpfa_current_image( $post_id ) {
 ...
 */
function qpfa_attachment_used_elsewhere( $att_id, $exclude_id = 0 ) {
	$att_id = (int) $att_id;
 ...
 */
function qpfa_backup_file( $file ) {
 ...
 * @return void
 */
function qpfa_delete_intermediate_files( $att_id ) {
 ...
 * @return array
 */
function qpfa_create_attachment( $source, $dir, $file, $post, $row ) {
	if ( ! wp_mkdir_p( $dir ) ) {
 ...
 * @return array گزارش.
 */
function qpfa_process_row( $row, $dry_run = false ) {
	...
	$report = array(
		...
		'backup' => '',
		'note'   => '',
		'alt'    => qpfa_alt_for( $row ),
		'kw'     => $row['kw'],
	);

	$source = qpfa_image_path( $row['file'] );
	...
	$current       = qpfa_current_image( $post->ID );
	...
	if ( $current['id'] && '' !== $current['file'] && $current['basename'] === $row['file'] ) {
		...
		if ( ! empty( $options['backup_old'] ) ) {
			$report['backup'] = qpfa_backup_file( $current['file'] );
		}

		// حذف فایل‌های اندازه‌های قبلی تا نسخهٔ قدیمی باقی نماند.
		qpfa_delete_intermediate_files( $current['id'] );
		...
		if ( ! empty( $options['set_alt'] ) ) {
			update_post_meta( $current['id'], '_wp_attachment_image_alt', qpfa_alt_for( $row ) );
			update_post_meta( $current['id'], '_qpfi_kw', $row['kw'] );
			update_post_meta( $current['id'], '_qpfi_alt_source', 'qp-fi-' . QPFA_VERSION );
		}
	...
	if ( $dry_run ) {
		...
		if ( $current['id'] ) {
			$others = qpfa_attachment_used_elsewhere( $current['id'], $post->ID );
	...
	list( $new_id, $error ) = qpfa_create_attachment( $source, $dir, $row['file'], $post, $row );
	...
	if ( ! empty( $options['set_alt'] ) ) {
		update_post_meta( $new_id, '_wp_attachment_image_alt', qpfa_alt_for( $row ) );
		update_post_meta( $new_id, '_qpfi_kw', $row['kw'] );
		update_post_meta( $new_id, '_qpfi_alt_source', 'qp-fi-' . QPFA_VERSION );
	}
...
function qpfa_plan() {
	...
	foreach ( qpfa_map() as $row ) {
		$plan[] = qpfa_process_row( $row, true );
	}
 ...
 * @return array
 */
function qpfa_run( $only_slugs = array() ) {
	$reports = array();
	...
	foreach ( qpfa_map() as $row ) {
		...
		$reports[] = qpfa_process_row( $row, false );
	}

	qpfa_clear_caches();
	update_option( QPFA_LOG, $reports, false );
 ...
 * @return array
 */
function qpfa_run_batch( $offset, $step = 0 ) {
	$map   = array_values( qpfa_map() );
	$total = count( $map );

	// اندازهٔ مرحله خودکار تنظیم می‌شود (هاست کند ← مرحلهٔ کوچک‌تر).
	if ( $step < 1 ) {
		$step = (int) get_option( 'qpfa_batch_size', QPFA_BATCH_SIZE );
	}
	...
	for ( $i = 0; $i < $step; $i++ ) {
		...
		$rows[] = qpfa_process_row( $map[ $index ], false );
	...
	update_option( 'qpfa_batch_size', $new_step, false );

	// از مرحلهٔ اول، گزارش قبلی پاک و از نو ساخته می‌شود.
	$stored = ( 0 === (int) $offset ) ? array() : get_option( QPFA_LOG, array() );

	if ( ! is_array( $stored ) ) {
		$stored = array();
	}

	update_option( QPFA_LOG, array_merge( $stored, $rows ), false );

	if ( $done ) {
		qpfa_clear_caches();
		delete_option( 'qpfa_batch_offset' );
	} else {
		update_option( 'qpfa_batch_offset', $next, false );
	}
 ...
 * @return string
 */
function qpfa_page_url( $args = array() ) {
	return add_query_arg( array_merge( array( 'page' => 'qpfa' ), $args ), admin_url( 'tools.php' ) );
}
