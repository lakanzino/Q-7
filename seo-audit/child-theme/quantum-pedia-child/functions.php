<?php
/**
 * Quantum Pedia Child — functions (v1.8.1)
 *
 ...
 * ۸. بخش «اصطلاحات کوانتومی» (inc/glossary-*): نوع محتوای qp_glossary با آرشیو
 *    /glossary/، اسلاگ انگلیسی اجباری، تولتیپ + لینک داخلی خودکار، سئوی DefinedTerm
 *
 * تغییرات 1.8.1 نسبت به 1.8.0:
 * - راه‌اندازی کامل بخش «اصطلاحات کوانتومی» در /glossary/ با اسلاگ انگلیسی
 *   (مدخل‌های قدیمی با اسلاگ فارسی به‌صورت خودکار ۳۰۱ می‌شوند)
 * - قالب‌های archive-qp_glossary.php و single-qp_glossary.php
 * - لینک «دانشمندان» و «اصطلاحات» در هدر + ورود اصطلاح‌ها به جست‌وجو و نقشهٔ سایت
 *
 * تغییرات 1.7.3 نسبت به قبل:
 * - همگام‌سازی بلوک‌های ویرایشگر و نسخه‌گذاری خودکار دارایی‌های بلوک
 * - اصلاح ایمنی ورودی‌های مسیر و کنترل وجود نوشته در متادیتای سئو
 *
 * تغییرات 1.7.2 نسبت به قبل:
 * - کش ناوبری هدر (رفع کندی بارگذاری برگه‌ها)
 * - بارگذاری شرطی CSS راهنما
 * - بهینه‌سازی find_page_url و نسخه‌ها
 *
...
define( 'QPEDIA_CHILD_VERSION', '1.8.1' );
...
/* پاک‌سازی کش ناوبری هدر هنگام تغییر دسته‌ها */
function qpedia_clear_header_nav_cache() {
	delete_transient( 'qpedia_header_nav_tree_v1' );
}
add_action( 'created_quantum_category', 'qpedia_clear_header_nav_cache' );
add_action( 'edited_quantum_category', 'qpedia_clear_header_nav_cache' );
add_action( 'delete_quantum_category', 'qpedia_clear_header_nav_cache' );
add_action( 'saved_term', 'qpedia_clear_header_nav_cache' );
...
function qpedia_child_enqueue_assets() {
	...
	// استایل راهنما فقط روی برگه‌هایی که احتمالاً نیاز دارند (کاهش حجم CSS در بقیه صفحات)
	$load_guide = is_page( array( 'start', 'از-کجا-شروع-کنیم' ) ) || is_page_template( 'page-homepage.php' );
	if ( $load_guide ) {
		$guide_path = get_stylesheet_directory() . '/assets/css/qpedia-guide.css';
		if ( file_exists( $guide_path ) ) {
			wp_enqueue_style(
				'qpedia-guide',
				get_stylesheet_directory_uri() . '/assets/css/qpedia-guide.css',
				array( 'qpedia-child-custom' ),
				filemtime( $guide_path )
			);
		}
	}
...
function qpedia_child_rewrite_rules() {
	...
	add_rewrite_rule(
		'^(?!scientists/?$)(?!glossary/?$)(?!topic/)(?!wp-admin/?$)(?!wp-json/?$)(?!feed/?$)(?!page/)(?!search/?$)(?!robots\\.txt$)(?!favicon\\.ico$)(?!xmlrpc\\.php$)(?!wp-login\\.php$)(?!sitemap.*\\.xml$)([a-z0-9][a-z0-9\\-]{2,})/?$',
...
/* ── ۹. واژه‌نامه: /glossary/ حالا آرشیو واقعی qp_glossary است (نسخهٔ 1.8.1) ──
   ریدایرکت قدیمی حذف شد چون مسیر رزروشده دیگر 404 نیست؛ آرشیو واژه‌نامه آن را می‌گیرد.
   ریدایرکت اسلاگ‌های فارسیِ نسل قبل در inc/glossary-post-type.php (§۵) انجام می‌شود. */
...
function qpedia_child_main_queries( $query ) {
	...
	// جست‌وجو: مقاله + دانشمند + اصطلاح + برگه.
	if ( $query->is_search() ) {
		$query->set( 'post_type', array( 'quantum_article', 'quantum_scientist', 'qp_glossary', 'page' ) );
		$query->set( 'posts_per_page', 12 );
		return;
	}
	...
	// آرشیو واژه‌نامه: همهٔ اصطلاح‌ها در یک صفحه (سئوی بهتر، بدون صفحه‌بندی نازک).
	if ( $query->is_post_type_archive( 'qp_glossary' ) ) {
...
function qpedia_child_find_page_url( $candidate_slugs, $fallback_path = '/' ) {
	...
	static $memo = array();
	$key = md5( implode( '|', $candidate_slugs ) . '|' . $fallback_path );
	if ( isset( $memo[ $key ] ) ) {
		return $memo[ $key ];
	}
...
/* ── ۱۵. بارگذاری ماژول واژه‌نامه — «اصطلاحات کوانتومی» (v1.8.1) ──
   ترتیب مهم است: نوع محتوا → کش → سئو → محتوا/شورت‌کدها → دارایی‌ها */
foreach ( array( 'glossary-post-type', 'glossary-cache', 'glossary-seo', 'glossary-content', 'glossary-assets' ) as $qpedia_inc ) {
...
function qpedia_auto_article_schema() {
	...
	$schema = array(
		...
		'publisher'        => array(
			...
			'logo'  => array(
				'@type' => 'ImageObject',
				'url'   => get_site_icon_url( 512 ) ?: get_stylesheet_directory_uri() . '/assets/images/qpedia-logo-neon.png',
			),
		),
	);
...
function qpedia_render_custom_sitemap() {
	...
	// آرشیو واژه‌نامهٔ اصطلاحات کوانتومی.
	$glossary_archive = get_post_type_archive_link( 'qp_glossary' );
	if ( $glossary_archive ) {
		echo "  <url>\n";
		echo '    <loc>' . esc_url( $glossary_archive ) . "</loc>\n";
		echo "    <changefreq>weekly</changefreq>\n";
		...
		foreach ( $terms as $term ) {
			if ( '1' === (string) get_post_meta( $term->ID, '_qp_term_noindex', true ) ) {
				continue;
			}

			echo "  <url>\n";
			echo '    <loc>' . esc_url( get_permalink( $term->ID ) ) . "</loc>\n";
			echo '    <lastmod>' . esc_html( get_the_modified_date( 'Y-m-d\TH:i:sP', $term->ID ) ) . "</lastmod>\n";
...
function qpedia_register_blocks() {
	$blocks_script_path = get_stylesheet_directory() . '/assets/js/blocks.js';
	$blocks_style_path  = get_stylesheet_directory() . '/assets/css/blocks-editor.css';

	wp_register_script(
		'qpedia-blocks',
		get_stylesheet_directory_uri() . '/assets/js/blocks.js',
		array( 'wp-blocks', 'wp-element', 'wp-block-editor', 'wp-components', 'wp-i18n' ),
		file_exists( $blocks_script_path ) ? filemtime( $blocks_script_path ) : QPEDIA_CHILD_VERSION,
		true
	);
	wp_register_style(
		...
		array(),
		file_exists( $blocks_style_path ) ? filemtime( $blocks_style_path ) : QPEDIA_CHILD_VERSION
	);
