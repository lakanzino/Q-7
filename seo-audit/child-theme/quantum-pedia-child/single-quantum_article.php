...
if ( ! function_exists( 'qpedia_prepare_article_content' ) ) {
	/**
	 * محتوا را از shortcodeهای قدیمی تمیز می‌کند و برای TOC آماده می‌سازد.
	 *
	...
	function qpedia_prepare_article_content( $content ) {
		...
		// بقایای shortcodeهای قدیمی را فقط اگر مستقل آمده باشند حذف کن.
		$content = preg_replace( '/^\s*\[(?:qpt[^\]]*|qterm[^\]]*|related[^\]]*)\]\s*/u', '', $content );

		$matches = array();
		preg_match_all( '/<(h[2-3])([^>]*)>(.*?)<\/\1>/isu', $content, $matches, PREG_SET_ORDER );
		...
		$content = preg_replace_callback(
			'/<(h[2-3])([^>]*)>(.*?)<\/\1>/isu',
			function ( $match ) use ( &$headings, &$used_ids, &$counter ) {
				$counter++;
				$tag   = strtolower( $match[1] );
				$attrs = $match[2];
				$title = trim( wp_strip_all_tags( $match[3] ) );

				if ( preg_match( '/\sid=(?:"([^"]+)"|\'([^\']+)\')/iu', $attrs, $id_match ) ) {
					$id = sanitize_title( $id_match[1] ?: $id_match[2] );
				} else {
				...
				$attrs = preg_replace( '/\s+id=(?:"[^"]*"|\'[^\']*\')/iu', '', $attrs );
				$headings[] = array(
					'level' => $tag,
					'id'    => $id,
					'title' => $title,
				...
				return '<' . $match[1] . $attrs . ' id="' . esc_attr( $id ) . '">' . $match[3] . '</' . $match[1] . '>';
			},
			$content
		);
		...
		foreach ( $headings as $heading ) {
			$link = '<a href="#' . esc_attr( $heading['id'] ) . '">' . esc_html( $heading['title'] ) . '</a>';
