#!/usr/bin/env python3
"""ساخت افزونه یک‌بارهٔ جایگزینی واژه‌نامه با ۲۰۰ اصطلاح تأییدشده."""
import json, zipfile
...
(SRC / 'qpedia-glossary-200-replacement.php').write_text(PHP)

readme = '''Qpedia One-Time Glossary Importer — 200 Terms (Replacement)
============================================================
...
zip_path = SRC.parent / 'qpedia-glossary-200-replacement.zip'
with zipfile.ZipFile( zip_path, 'w', zipfile.ZIP_DEFLATED ) as z:
    for name in ( 'qpedia-glossary-200-replacement.php', 'terms.json', 'README.txt' ):
        z.write( SRC / name, 'qpedia-glossary-200-replacement/' + name )

terms = json.loads( ( SRC / 'terms.json' ).read_text() )
print( json.dumps( { 'terms': len( terms ), 'zip': str( zip_path.relative_to( ROOT ) ) }, ensure_ascii=False ) )
