#!/usr/bin/env python3
"""Build importer plugin for incomplete-depth-final-11."""
...
IMP_DIR = BASE / 'importers' / 'incomplete-depth-final-11'
PLUGIN_DIR = IMP_DIR / 'qpedia-importer-incomplete-depth-final-11'
PLUGIN_DIR.mkdir(parents=True, exist_ok=True)

folders = [
    '167-entanglement-quantum-computers',
    '168-quantum-learning-resources',
    '169-solar-cells-photoelectric',
    '170-why-quantum-math-works',
    '171-quantum-computer-reality',
    '173-quark',
    '174-higgs-boson',
    '175-antimatter',
    '176-nuclear-spin',
...
php_code = """<?php
/**
 * Plugin Name: Qpedia One-Time Importer Incomplete Depth Final 11
 * Description: به‌روزرسانی کنترل‌شده یازده مقاله پایانی گروه ناقص Qpedia با SEO (سازگار با Rank Math و TamRank)، دسته‌بندی و FAQ Schema.
 * Version: 1.0.0
...
const QPID11_FINAL_OPTION = 'qpedia_importer_incomplete_depth_final_11_completed';

add_action('admin_menu', function() {
    add_management_page(
        'درون‌ریز ۱۱ مقاله پایانی Qpedia',
        'درون‌ریز ۱۱ مقاله پایانی Qpedia',
        'manage_options',
        'qpedia-importer-incomplete-depth-final-11',
        'qpid11_final_page'
    );
});

function qpid11_final_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QPID11_FINAL_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز ۱۱ مقاله نهایی گروه ناقص بازنویسی‌شده Qpedia</h1>';
    
    if (isset($_POST['qpid11_final_run'])) {
        check_admin_referer('qpid11_final_run');
        $r = qpid11_final_run();
        $bad = false;
        ...
        echo '</ul></div>';
        if (!$bad) {
            update_option(QPID11_FINAL_OPTION, current_time('mysql'), false);
            echo '<p><strong>هر ۱۱ مقاله با موفقیت به‌روزرسانی شدند؛ اکنون می‌توانید افزونه را غیرفعال و حذف کنید.</strong></p></div>';
            return;
        }
    }
    ...
    echo '<p>این افزونه یازده مقاله موجود را با ID یا slug به‌روزرسانی می‌کند. کلیدواژه‌ها، عنوان و توضیحات متا برای TamRank SEO و Rank Math، دسته‌بندی‌ها، اسکیما و متن کامل ثبت می‌شوند. تاریخ انتشار قدیمی، نویسنده و تصویر شاخص تغییر نمی‌کنند.</p>';
    echo '<form method="post">';
    wp_nonce_field('qpid11_final_run');
    submit_button('اجرای به‌روزرسانی ۱۱ مقاله', 'primary', 'qpid11_final_run');
    echo '</form></div>';
}

function qpid11_final_run() {
    ...
    if (!is_array($items) || count($items) !== 11) return [['ok' => false, 'message' => 'فایل داده معتبر نیست؛ انتظار ۱۱ مقاله است.']];
...
(PLUGIN_DIR / 'qpedia-importer-incomplete-depth-final-11.php').write_text(php_code.strip(), encoding='utf-8')

readme = """=== Qpedia One-Time Importer Incomplete Depth Final 11 ===
...
== Description ==
افزونه یک‌بارمصرف برای به‌روزرسانی ۱۱ مقاله نهایی گروه ناقص Qpedia (شامل مقاله‌های ۱۶۷ تا ۱۸۱) با متادیتای سئو، کلیدواژه‌ها، عنوان و توضیحات متا برای TamRank SEO و Rank Math، اسکیما و لینک‌های داخلی.
"""
(PLUGIN_DIR / 'README.txt').write_text(readme.strip(), encoding='utf-8')

# Zip packaging
zip_path = IMP_DIR / 'qpedia-importer-incomplete-depth-final-11.zip'
