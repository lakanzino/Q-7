#!/usr/bin/env python3
"""Split 56 featured images into 5-article batch WordPress plugins (Batches 01 to 12)."""
import json
import os
import shutil
import zipfile
...
SRC_IMG_DIR = ROOT / 'article-rewrite-2026-09-22/featured-images-56-webp'
OUT_BASE = ROOT / 'article-rewrite-2026-09-22/importers/featured-images-batches'
...
print(f'Total items to batch: {len(items)}')
...
for idx, b_items in enumerate(batches, 1):
    ...
    # Copy webp images
    for item in b_items:
        src_img = SRC_IMG_DIR / item['filename']
        assert src_img.exists(), f'Missing {src_img}'
        shutil.copy2(src_img, img_dir / item['filename'])
        
    # Write json data
    (b_dir / 'images_data.json').write_text(json.dumps(b_items, ensure_ascii=False, indent=2), encoding='utf-8')
    
    # Build list of items in HTML for admin UI
    items_html_lines = []
    for it in b_items:
 ...
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره {idx:02d} شامل {len(b_items)} مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: {b_slug}
 */
...
function {b_func}_render_page() {{
    ...
    echo '<h2>فهرست مقالات و تصاویر این بسته ({len(b_items)} تصویر):</h2>';
...
'''
    ...
    created_zips.append((idx, b_slug, len(b_items), dl_zip.stat().st_size / 1024))
    print(f'Created Batch {idx:02d}: {b_slug}.zip ({dl_zip.stat().st_size / 1024:.1f} KB, {len(b_items)} items)')

print('All 12 batches generated successfully!')
