<?php
/**
 * Plugin Name: Qpedia One-Time Importer 076–085
 * Description: جایگزینی کنترل‌شده مقاله‌های ۷۶ تا ۸۵ Qpedia و ثبت متادیتای Rank Math؛ فقط یک بار قابل اجرا است.
 * Version: 1.0.0
 * Author: Qpedia Editorial
 */
if (!defined('ABSPATH')) exit;

const QP7685_OPTION = 'qpedia_importer_076_085_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 076–085', 'درون‌ریز Qpedia 076–085', 'manage_options', 'qpedia-importer-076-085', 'qp7685_render_page');
});

function qp7685_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP7685_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۷۶ تا ۸۵</h1>';
    if ($done) {
        echo '<div class="notice notice-success"><p>این بسته قبلاً در '.esc_html($done).' اجرا شده است و دوباره اجرا نمی‌شود.</p></div></div>';
        return;
    }
    if (isset($_POST['qp7685_run'])) {
        check_admin_referer('qp7685_run_import');
        $results = qp7685_run_import();
        $has_error = false;
        ...
        echo '</ul></div>';
        if (!$has_error) {
            update_option(QP7685_OPTION, current_time('mysql'), false);
    ...
    wp_nonce_field('qp7685_run_import');
    submit_button('به‌روزرسانی مقاله‌های ۷۶ تا ۸۵', 'primary', 'qp7685_run');
    echo '</form></div>';
}

function qp7685_run_import() {
