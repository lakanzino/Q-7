<?php
/**
 * Plugin Name: Qpedia One-Time Importer 001–025
 * Description: جایگزینی کنترل‌شده مقاله‌های یازدهم تا پانزدهم Qpedia و ثبت متادیتای Rank Math؛ فقط یک بار قابل اجرا است.
 * Version: 1.0.0
 * Author: Qpedia Editorial
 */
if (!defined('ABSPATH')) exit;

const QP5_OPTION = 'qpedia_importer_001_025_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 001–025', 'درون‌ریز Qpedia 001–025', 'manage_options', 'qpedia-importer-001-025', 'qp5_render_page');
});

function qp5_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP5_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۱ تا ۲۵</h1>';
    if ($done) {
    ...
    if (isset($_POST['qp5_run'])) {
        ...
        if (!$has_error) {
            ...
            echo '<p><strong>هر ۲۵ مقاله به‌روزرسانی شدند. افزونه را اکنون غیرفعال و حذف کنید.</strong></p></div>';
            return;
        }
    }
    ...
    submit_button('به‌روزرسانی مقاله‌های ۱ تا ۲۵', 'primary', 'qp5_run');
    echo '</form></div>';
}
...
function qp5_run_import() {
    ...
    if (!is_array($items) || count($items) !== 25) return array(array('ok'=>false, 'message'=>'خطا: فایل داده معتبر نیست.'));
    $results = array();
