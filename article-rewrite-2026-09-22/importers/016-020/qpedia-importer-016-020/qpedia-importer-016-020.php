<?php
/**
 * Plugin Name: Qpedia One-Time Importer 016–020
...
const QP5_OPTION = 'qpedia_importer_016_020_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 016–020', 'درون‌ریز Qpedia 016–020', 'manage_options', 'qpedia-importer-016-020', 'qp5_render_page');
});

function qp5_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP5_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۱۶ تا ۲۰</h1>';
    if ($done) {
    ...
    if (isset($_POST['qp5_run'])) {
        ...
        if (!$has_error) {
            ...
            echo '<p><strong>هر پنج مقاله به‌روزرسانی شدند. افزونه را اکنون غیرفعال و حذف کنید.</strong></p></div>';
            return;
        }
    }
    ...
    submit_button('به‌روزرسانی مقاله‌های ۱۶ تا ۲۰', 'primary', 'qp5_run');
    echo '</form></div>';
}
