<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 09 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره 09 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-09
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b09_admin_menu');
function qp_fimg_b09_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 09',
        'تصاویر شاخص بسته 09',
        'manage_options',
        'qpedia-featured-images-batch-09',
        'qp_fimg_b09_render_page'
    );
}

function qp_fimg_b09_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 09</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_9');
        $result = qp_fimg_b09_execute();
    ...
    echo '<li><strong>اثر آهارونوف-بوهم</strong> (<code>aharonov-bohm-effect</code>) &larr; <code>aharonov-bohm-effect.webp</code></li>';
    ...
    echo '<li><strong>کرم‌چاله</strong> (<code>wormhole</code>) &larr; <code>wormhole.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_9');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 09', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b09_execute() {
    ...
    $msg = 'عملیات بسته ' . 9 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
