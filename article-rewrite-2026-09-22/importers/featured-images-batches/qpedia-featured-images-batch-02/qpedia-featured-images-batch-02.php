<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 02 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره 02 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-02
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b02_admin_menu');
function qp_fimg_b02_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 02',
        'تصاویر شاخص بسته 02',
        'manage_options',
        'qpedia-featured-images-batch-02',
        'qp_fimg_b02_render_page'
    );
}

function qp_fimg_b02_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 02</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_2');
        $result = qp_fimg_b02_execute();
    ...
    echo '<li><strong>ثابت پلانک چیست؟</strong> (<code>planck-constant</code>) &larr; <code>planck-constant.webp</code></li>';
    ...
    wp_nonce_field('qp_attach_nonce_2');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 02', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b02_execute() {
    ...
    $msg = 'عملیات بسته ' . 2 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
