<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 11 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره 11 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-11
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b11_admin_menu');
function qp_fimg_b11_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 11',
        'تصاویر شاخص بسته 11',
        'manage_options',
        'qpedia-featured-images-batch-11',
        'qp_fimg_b11_render_page'
    );
}

function qp_fimg_b11_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 11</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_11');
        $result = qp_fimg_b11_execute();
    ...
    echo '<li><strong>داروینیسم کوانتومی</strong> (<code>quantum-darwinism</code>) &larr; <code>quantum-darwinism.webp</code></li>';
    echo '<li><strong>کوانتوم و اراده آزاد</strong> (<code>quantum-free-will</code>) &larr; <code>quantum-free-will.webp</code></li>';
    echo '<li><strong>سفر در زمان</strong> (<code>quantum-time-travel</code>) &larr; <code>quantum-time-travel.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_11');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 11', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b11_execute() {
    ...
    $msg = 'عملیات بسته ' . 11 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
