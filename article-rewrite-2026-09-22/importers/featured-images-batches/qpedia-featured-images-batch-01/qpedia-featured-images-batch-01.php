<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 01 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره 01 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-01
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b01_admin_menu');
function qp_fimg_b01_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 01',
        'تصاویر شاخص بسته 01',
        'manage_options',
        'qpedia-featured-images-batch-01',
        'qp_fimg_b01_render_page'
    );
}

function qp_fimg_b01_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 01</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_1');
        $result = qp_fimg_b01_execute();
    ...
    echo '<li><strong>دوگانگی موج و ذره</strong> (<code>wave-particle-duality</code>) &larr; <code>wave-particle-duality.webp</code></li>';
    echo '<li><strong>کوانتوم یعنی چه؟</strong> (<code>what-is-quantum</code>) &larr; <code>what-is-quantum.webp</code></li>';
    ...
    wp_nonce_field('qp_attach_nonce_1');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 01', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b01_execute() {
    ...
    $msg = 'عملیات بسته ' . 1 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
