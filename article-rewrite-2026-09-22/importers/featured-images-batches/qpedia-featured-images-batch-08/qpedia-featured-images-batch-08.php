<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 08 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره 08 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-08
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b08_admin_menu');
function qp_fimg_b08_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 08',
        'تصاویر شاخص بسته 08',
        'manage_options',
        'qpedia-featured-images-batch-08',
        'qp_fimg_b08_render_page'
    );
}

function qp_fimg_b08_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 08</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_8');
        $result = qp_fimg_b08_execute();
    ...
    echo '<li><strong>برهم‌نهی اشیای بزرگ</strong> (<code>why-large-objects-dont-superpose</code>) &larr; <code>why-large-objects-dont-superpose.webp</code></li>';
    echo '<li><strong>اصل عدم قطعیت هایزنبرگ</strong> (<code>heisenberg-uncertainty-principle</code>) &larr; <code>heisenberg-uncertainty-principle.webp</code></li>';
    echo '<li><strong>آیا فیزیک کلاسیک اشتباه بود؟</strong> (<code>is-classical-physics-wrong</code>) &larr; <code>is-classical-physics-wrong.webp</code></li>';
    echo '<li><strong>اصل طرد پاولی</strong> (<code>pauli-exclusion-principle</code>) &larr; <code>pauli-exclusion-principle.webp</code></li>';
    echo '<li><strong>ماکس بورن و تفسیر احتمالاتی</strong> (<code>quantum-interpretation-debate</code>) &larr; <code>quantum-interpretation-debate.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_8');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 08', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b08_execute() {
    ...
    $msg = 'عملیات بسته ' . 8 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
