<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 07 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP (بسته شماره 07 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-07
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b07_admin_menu');
function qp_fimg_b07_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 07',
        'تصاویر شاخص بسته 07',
        'manage_options',
        'qpedia-featured-images-batch-07',
        'qp_fimg_b07_render_page'
    );
}

function qp_fimg_b07_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 07</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_7');
        $result = qp_fimg_b07_execute();
    ...
    echo '<h2>فهرست مقالات و تصاویر این بسته (5 تصویر):</h2>';
    echo '<ol style="font-size:14px;line-height:1.9;">';
    echo '<li><strong>عدد کوانتومی</strong> (<code>quantum-number</code>) &larr; <code>quantum-number.webp</code></li>';
    echo '<li><strong>تمثیل تاس در مقابل تمثیل سکه</strong> (<code>coin-vs-dice-quantum-uncertainty</code>) &larr; <code>coin-vs-dice-quantum-uncertainty.webp</code></li>';
    echo '<li><strong>فیزیک کوانتوم و مکانیک کوانتومی</strong> (<code>quantum-physics-vs-quantum-mechanics</code>) &larr; <code>quantum-physics-vs-quantum-mechanics.webp</code></li>';
    echo '<li><strong>نیلز بور و اصل تکمیل</strong> (<code>quantum-analogy-exercise-boundary</code>) &larr; <code>quantum-analogy-exercise-boundary.webp</code></li>';
    echo '<li><strong>جبرگرایی کلاسیک و احتمال کوانتومی</strong> (<code>determinism-vs-probability</code>) &larr; <code>determinism-vs-probability.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_7');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 07', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b07_execute() {
    ...
    $msg = 'عملیات بسته ' . 7 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
