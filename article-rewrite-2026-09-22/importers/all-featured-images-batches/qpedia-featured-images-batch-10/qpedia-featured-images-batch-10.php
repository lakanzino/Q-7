<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 10 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 10 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-10
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b10_admin_menu');
function qp_fimg_b10_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 10',
        'تصاویر شاخص بسته 10',
        'manage_options',
        'qpedia-featured-images-batch-10',
        'qp_fimg_b10_render_page'
    );
}

function qp_fimg_b10_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 10</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_10');
        $result = qp_fimg_b10_execute();
    ...
    echo '<li><strong>اصل هولوگرافیک</strong> (<code>holographic-principle</code>) &larr; <code>holographic-principle.webp</code></li>';
    echo '<li><strong>آشکارسازی هوموداین با سیگنال و نوسان‌ساز محلی</strong> (<code>homodyne-detection</code>) &larr; <code>homodyne-detection.webp</code></li>';
    echo '<li><strong>درهم‌تنیدگی و سرعت نور</strong> (<code>human-teleportation</code>) &larr; <code>human-teleportation.webp</code></li>';
    ...
    wp_nonce_field('qp_attach_nonce_10');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 10', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b10_execute() {
    ...
    $msg = 'عملیات بسته ' . 10 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
