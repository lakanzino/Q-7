<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 24 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 24 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-24
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b24_admin_menu');
function qp_fimg_b24_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 24',
        'تصاویر شاخص بسته 24',
        'manage_options',
        'qpedia-featured-images-batch-24',
        'qp_fimg_b24_render_page'
    );
}

function qp_fimg_b24_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 24</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_24');
        $result = qp_fimg_b24_execute();
    ...
    echo '<li><strong>دیو ماکسول کوانتومی</strong> (<code>quantum-maxwell-demon</code>) &larr; <code>quantum-maxwell-demon.webp</code></li>';
    echo '<li><strong>اندازه‌گیری و فروپاشی</strong> (<code>quantum-measurement</code>) &larr; <code>quantum-measurement.webp</code></li>';
    echo '<li><strong>میکروسکوپ کوانتومی و تصویربرداری نمونه</strong> (<code>quantum-microscopy</code>) &larr; <code>quantum-microscopy.webp</code></li>';
    echo '<li><strong>پول کوانتومی</strong> (<code>quantum-money</code>) &larr; <code>quantum-money.webp</code></li>';
    echo '<li><strong>موسیقی کوانتومی و تبدیل داده کوانتومی به صدا</strong> (<code>quantum-music</code>) &larr; <code>quantum-music.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_24');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 24', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b24_execute() {
    ...
    $msg = 'عملیات بسته ' . 24 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
