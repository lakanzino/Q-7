<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 13 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 13 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-13
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b13_admin_menu');
function qp_fimg_b13_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 13',
        'تصاویر شاخص بسته 13',
        'manage_options',
        'qpedia-featured-images-batch-13',
        'qp_fimg_b13_render_page'
    );
}

function qp_fimg_b13_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 13</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_13');
        $result = qp_fimg_b13_execute();
    ...
    echo '<li><strong>ناظر در کوانتوم</strong> (<code>observer</code>) &larr; <code>observer.webp</code></li>';
    echo '<li><strong>شبکه نوری برای به‌دام‌اندازی اتم‌ها</strong> (<code>optical-lattice</code>) &larr; <code>optical-lattice.webp</code></li>';
    echo '<li><strong>نوسانگر پارامتری نوری و تولید سیگنال و آیدلر</strong> (<code>optical-parametric-oscillator</code>) &larr; <code>optical-parametric-oscillator.webp</code></li>';
    echo '<li><strong>اصل طرد پاولی</strong> (<code>pauli-exclusion-principle</code>) &larr; <code>pauli-exclusion-principle.webp</code></li>';
    echo '<li><strong>فونون و ارتعاش شبکه بلوری</strong> (<code>phonon</code>) &larr; <code>phonon.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_13');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 13', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b13_execute() {
    ...
    $msg = 'عملیات بسته ' . 13 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
