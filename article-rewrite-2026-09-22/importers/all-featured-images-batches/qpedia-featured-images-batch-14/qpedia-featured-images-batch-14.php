<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 14 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 14 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-14
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b14_admin_menu');
function qp_fimg_b14_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 14',
        'تصاویر شاخص بسته 14',
        'manage_options',
        'qpedia-featured-images-batch-14',
        'qp_fimg_b14_render_page'
    );
}

function qp_fimg_b14_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 14</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_14');
        $result = qp_fimg_b14_execute();
    ...
    echo '<li><strong>انسداد فوتونی و عبور یک فوتون</strong> (<code>photon-blockade</code>) &larr; <code>photon-blockade.webp</code></li>';
    echo '<li><strong>شمارش فوتون و توزیع تعداد رویدادها</strong> (<code>photon-counting</code>) &larr; <code>photon-counting.webp</code></li>';
    echo '<li><strong>ثابت پلانک چیست؟</strong> (<code>planck-constant</code>) &larr; <code>planck-constant.webp</code></li>';
    echo '<li><strong>پلاسمون و نوسان جمعی الکترون‌ها</strong> (<code>plasmon</code>) &larr; <code>plasmon.webp</code></li>';
    echo '<li><strong>پولاریتون حاصل برهم‌کنش نور و ماده</strong> (<code>polariton</code>) &larr; <code>polariton.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_14');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 14', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b14_execute() {
    ...
    $msg = 'عملیات بسته ' . 14 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
