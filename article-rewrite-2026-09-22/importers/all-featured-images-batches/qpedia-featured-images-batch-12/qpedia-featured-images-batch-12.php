<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 12 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 12 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-12
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b12_admin_menu');
function qp_fimg_b12_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 12',
        'تصاویر شاخص بسته 12',
        'manage_options',
        'qpedia-featured-images-batch-12',
        'qp_fimg_b12_render_page'
    );
}

function qp_fimg_b12_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 12</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_12');
        $result = qp_fimg_b12_execute();
    ...
    echo '<li><strong>جایگزیدگی چندجسمی</strong> (<code>many-body-localization</code>) &larr; <code>many-body-localization.webp</code></li>';
    echo '<li><strong>توزیع کلید کوانتومی مستقل از دستگاه اندازه‌گیری</strong> (<code>measurement-device-independent-qkd</code>) &larr; <code>measurement-device-independent-qkd.webp</code></li>';
    echo '<li><strong>عایق مات</strong> (<code>mott-insulator</code>) &larr; <code>mott-insulator.webp</code></li>';
    echo '<li><strong>نانوسیم و ساختارهای نانومتری</strong> (<code>nanowire</code>) &larr; <code>nanowire.webp</code></li>';
    echo '<li><strong>قضیهٔ عدم‌کپی</strong> (<code>no-cloning-theorem</code>) &larr; <code>no-cloning-theorem.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_12');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 12', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b12_execute() {
    ...
    $msg = 'عملیات بسته ' . 12 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
