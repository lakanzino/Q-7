<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 27 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 27 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-27
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b27_admin_menu');
function qp_fimg_b27_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 27',
        'تصاویر شاخص بسته 27',
        'manage_options',
        'qpedia-featured-images-batch-27',
        'qp_fimg_b27_render_page'
    );
}

function qp_fimg_b27_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 27</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_27');
        $result = qp_fimg_b27_execute();
    ...
    echo '<li><strong>یخچال کوانتومی</strong> (<code>quantum-refrigerator</code>) &larr; <code>quantum-refrigerator.webp</code></li>';
    echo '<li><strong>یادگیری تقویتی کوانتومی</strong> (<code>quantum-reinforcement-learning</code>) &larr; <code>quantum-reinforcement-learning.webp</code></li>';
    echo '<li><strong>مسیریاب کوانتومی</strong> (<code>quantum-router</code>) &larr; <code>quantum-router.webp</code></li>';
    echo '<li><strong>اشتراک راز کوانتومی</strong> (<code>quantum-secret-sharing</code>) &larr; <code>quantum-secret-sharing.webp</code></li>';
    echo '<li><strong>ارتباط مستقیم امن کوانتومی میان فرستنده و گیرنده</strong> (<code>quantum-secure-direct-communication</code>) &larr; <code>quantum-secure-direct-communication.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_27');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 27', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b27_execute() {
    ...
    $msg = 'عملیات بسته ' . 27 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
