<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 34 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 34 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-34
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b34_admin_menu');
function qp_fimg_b34_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 34',
        'تصاویر شاخص بسته 34',
        'manage_options',
        'qpedia-featured-images-batch-34',
        'qp_fimg_b34_render_page'
    );
}

function qp_fimg_b34_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 34</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_34');
        $result = qp_fimg_b34_execute();
    ...
    echo '<li><strong>نیمه‌فلز ویل</strong> (<code>weyl-semimetal</code>) &larr; <code>weyl-semimetal.webp</code></li>';
    ...
    echo '<li><strong>دوست ویگنر</strong> (<code>wigner-friend</code>) &larr; <code>wigner-friend.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_34');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 34', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b34_execute() {
    ...
    $msg = 'عملیات بسته ' . 34 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
