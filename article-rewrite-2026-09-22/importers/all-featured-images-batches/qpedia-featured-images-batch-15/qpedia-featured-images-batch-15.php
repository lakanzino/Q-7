<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 15 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 15 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-15
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b15_admin_menu');
function qp_fimg_b15_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 15',
        'تصاویر شاخص بسته 15',
        'manage_options',
        'qpedia-featured-images-batch-15',
        'qp_fimg_b15_render_page'
    );
}

function qp_fimg_b15_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 15</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_15');
        $result = qp_fimg_b15_execute();
    ...
    echo '<li><strong>پولارون در شبکه بلوری</strong> (<code>polaron</code>) &larr; <code>polaron.webp</code></li>';
    echo '<li><strong>واپاشی پروتون</strong> (<code>proton-decay</code>) &larr; <code>proton-decay.webp</code></li>';
    echo '<li><strong>الگوریتم بهینه‌سازی تقریبی کوانتومی</strong> (<code>qaoa</code>) &larr; <code>qaoa.webp</code></li>';
    ...
    echo '<li><strong>بیهوشی و فرضیه‌های کوانتومی</strong> (<code>quantum-anesthesia</code>) &larr; <code>quantum-anesthesia.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_15');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 15', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b15_execute() {
    ...
    $msg = 'عملیات بسته ' . 15 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
