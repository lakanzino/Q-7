<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 35 (2 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 35 شامل 2 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-35
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b35_admin_menu');
function qp_fimg_b35_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 35',
        'تصاویر شاخص بسته 35',
        'manage_options',
        'qpedia-featured-images-batch-35',
        'qp_fimg_b35_render_page'
    );
}

function qp_fimg_b35_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 35</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_35');
        $result = qp_fimg_b35_execute();
    ...
    echo '<h2>فهرست مقالات و تصاویر این بسته (2 تصویر با نشان qpedia.ir):</h2>';
    echo '<ol style="font-size:14px;line-height:1.9;">';
    echo '<li><strong>ذره فرضی ویمپ</strong> (<code>wimp</code>) &larr; <code>wimp.webp</code></li>';
    echo '<li><strong>کرم‌چاله</strong> (<code>wormhole</code>) &larr; <code>wormhole.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_35');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 35', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b35_execute() {
    ...
    $msg = 'عملیات بسته ' . 35 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
