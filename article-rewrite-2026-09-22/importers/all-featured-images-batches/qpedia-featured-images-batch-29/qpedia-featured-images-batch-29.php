<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 29 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 29 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-29
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b29_admin_menu');
function qp_fimg_b29_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 29',
        'تصاویر شاخص بسته 29',
        'manage_options',
        'qpedia-featured-images-batch-29',
        'qp_fimg_b29_render_page'
    );
}

function qp_fimg_b29_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 29</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_29');
        $result = qp_fimg_b29_execute();
    ...
    echo '<li><strong>گرمایی‌شدن کوانتومی</strong> (<code>quantum-thermalization</code>) &larr; <code>quantum-thermalization.webp</code></li>';
    echo '<li><strong>دماسنجی کوانتومی</strong> (<code>quantum-thermometry</code>) &larr; <code>quantum-thermometry.webp</code></li>';
    echo '<li><strong>سفر در زمان</strong> (<code>quantum-time-travel</code>) &larr; <code>quantum-time-travel.webp</code></li>';
    echo '<li><strong>تونل‌زنی کوانتومی</strong> (<code>quantum-tunneling</code>) &larr; <code>quantum-tunneling.webp</code></li>';
    echo '<li><strong>تصویر مفهومی کوانتوم در ویروس‌ها</strong> (<code>quantum-viruses</code>) &larr; <code>quantum-viruses.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_29');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 29', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b29_execute() {
    ...
    $msg = 'عملیات بسته ' . 29 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
