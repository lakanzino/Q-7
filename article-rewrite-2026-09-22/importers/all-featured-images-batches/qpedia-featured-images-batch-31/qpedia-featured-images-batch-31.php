<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 31 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 31 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-31
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b31_admin_menu');
function qp_fimg_b31_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 31',
        'تصاویر شاخص بسته 31',
        'manage_options',
        'qpedia-featured-images-batch-31',
        'qp_fimg_b31_render_page'
    );
}

function qp_fimg_b31_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 31</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_31');
        $result = qp_fimg_b31_execute();
    ...
    echo '<li><strong>اتم ریدبرگ و ترازهای انرژی</strong> (<code>rydberg-atom</code>) &larr; <code>rydberg-atom.webp</code></li>';
    echo '<li><strong>نمایش الگوریتم سایمون</strong> (<code>simon-algorithm</code>) &larr; <code>simon-algorithm.webp</code></li>';
    echo '<li><strong>نور کند در محیط پاشنده</strong> (<code>slow-light</code>) &larr; <code>slow-light.webp</code></li>';
    echo '<li><strong>سالیتون و حفظ شکل موج هنگام انتشار</strong> (<code>soliton</code>) &larr; <code>soliton.webp</code></li>';
    echo '<li><strong>ساختار یخ اسپینی</strong> (<code>spin-ice</code>) &larr; <code>spin-ice.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_31');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 31', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b31_execute() {
    ...
    $msg = 'عملیات بسته ' . 31 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
