<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 22 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 22 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-22
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b22_admin_menu');
function qp_fimg_b22_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 22',
        'تصاویر شاخص بسته 22',
        'manage_options',
        'qpedia-featured-images-batch-22',
        'qp_fimg_b22_render_page'
    );
}

function qp_fimg_b22_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 22</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_22');
        $result = qp_fimg_b22_execute();
    ...
    echo '<li><strong>نظریه بازی کوانتومی</strong> (<code>quantum-game-theory</code>) &larr; <code>quantum-game-theory.webp</code></li>';
    echo '<li><strong>بازی‌های کوانتومی</strong> (<code>quantum-games</code>) &larr; <code>quantum-games.webp</code></li>';
    echo '<li><strong>مدل مولد کوانتومی</strong> (<code>quantum-generative-model</code>) &larr; <code>quantum-generative-model.webp</code></li>';
    echo '<li><strong>موتور گرمایی کوانتومی</strong> (<code>quantum-heat-engine</code>) &larr; <code>quantum-heat-engine.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_22');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 22', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b22_execute() {
    ...
    $msg = 'عملیات بسته ' . 22 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
