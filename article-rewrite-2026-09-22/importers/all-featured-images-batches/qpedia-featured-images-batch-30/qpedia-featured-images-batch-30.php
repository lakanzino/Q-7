<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 30 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 30 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-30
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b30_admin_menu');
function qp_fimg_b30_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 30',
        'تصاویر شاخص بسته 30',
        'manage_options',
        'qpedia-featured-images-batch-30',
        'qp_fimg_b30_render_page'
    );
}

function qp_fimg_b30_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 30</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_30');
        $result = qp_fimg_b30_execute();
    ...
    echo '<li><strong>رأی‌گیری کوانتومی</strong> (<code>quantum-voting</code>) &larr; <code>quantum-voting.webp</code></li>';
    echo '<li><strong>مقایسه قدم‌زدن کلاسیک و کوانتومی</strong> (<code>quantum-walk</code>) &larr; <code>quantum-walk.webp</code></li>';
    echo '<li><strong>زمستان کوانتومی</strong> (<code>quantum-winter</code>) &larr; <code>quantum-winter.webp</code></li>';
    echo '<li><strong>اثر زنون کوانتومی</strong> (<code>quantum-zeno-effect</code>) &larr; <code>quantum-zeno-effect.webp</code></li>';
    echo '<li><strong>نمودار نوسانات رابی در سامانه دوترازی</strong> (<code>rabi-oscillations</code>) &larr; <code>rabi-oscillations.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_30');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 30', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b30_execute() {
    ...
    $msg = 'عملیات بسته ' . 30 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
