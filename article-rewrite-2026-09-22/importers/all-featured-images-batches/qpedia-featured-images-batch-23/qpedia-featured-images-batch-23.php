<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 23 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 23 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-23
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b23_admin_menu');
function qp_fimg_b23_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 23',
        'تصاویر شاخص بسته 23',
        'manage_options',
        'qpedia-featured-images-batch-23',
        'qp_fimg_b23_render_page'
    );
}

function qp_fimg_b23_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 23</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_23');
        $result = qp_fimg_b23_execute();
    ...
    echo '<li><strong>تصویربرداری کوانتومی با فوتون‌های آشکارنشده</strong> (<code>quantum-imaging-undetected-photons</code>) &larr; <code>quantum-imaging-undetected-photons.webp</code></li>';
    ...
    echo '<li><strong>هسته کوانتومی در یادگیری ماشین</strong> (<code>quantum-kernel</code>) &larr; <code>quantum-kernel.webp</code></li>';
    echo '<li><strong>لیتوگرافی کوانتومی و ایجاد الگوی ریز</strong> (<code>quantum-lithography</code>) &larr; <code>quantum-lithography.webp</code></li>';
    echo '<li><strong>منطق کوانتومی و مدار منطقی</strong> (<code>quantum-logic</code>) &larr; <code>quantum-logic.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_23');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 23', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b23_execute() {
    ...
    $msg = 'عملیات بسته ' . 23 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
