<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 21 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 21 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-21
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b21_admin_menu');
function qp_fimg_b21_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 21',
        'تصاویر شاخص بسته 21',
        'manage_options',
        'qpedia-featured-images-batch-21',
        'qp_fimg_b21_render_page'
    );
}

function qp_fimg_b21_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 21</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_21');
        $result = qp_fimg_b21_execute();
    ...
    echo '<li><strong>داستان علمی‌تخیلی کوانتومی و جهان‌های روایی</strong> (<code>quantum-fiction</code>) &larr; <code>quantum-fiction.webp</code></li>';
    echo '<li><strong>محاسبات کوانتومی در وال‌استریت</strong> (<code>quantum-finance</code>) &larr; <code>quantum-finance.webp</code></li>';
    echo '<li><strong>انگشت‌نگاری کوانتومی برای مقایسه داده‌ها</strong> (<code>quantum-fingerprinting</code>) &larr; <code>quantum-fingerprinting.webp</code></li>';
    echo '<li><strong>قضیه نوسان کوانتومی</strong> (<code>quantum-fluctuation-theorem</code>) &larr; <code>quantum-fluctuation-theorem.webp</code></li>';
    echo '<li><strong>FPGA کوانتومی و کنترل بازپیکربندی‌پذیر</strong> (<code>quantum-fpga</code>) &larr; <code>quantum-fpga.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_21');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 21', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b21_execute() {
    ...
    $msg = 'عملیات بسته ' . 21 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
