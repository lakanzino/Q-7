<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 04 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 04 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-04
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b04_admin_menu');
function qp_fimg_b04_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 04',
        'تصاویر شاخص بسته 04',
        'manage_options',
        'qpedia-featured-images-batch-04',
        'qp_fimg_b04_render_page'
    );
}

function qp_fimg_b04_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 04</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_4');
        $result = qp_fimg_b04_execute();
    ...
    echo '<li><strong>اصل مکملیت</strong> (<code>complementarity-principle</code>) &larr; <code>complementarity-principle.webp</code></li>';
    echo '<li><strong>تفسیر کپنهاگی</strong> (<code>copenhagen-interpretation</code>) &larr; <code>copenhagen-interpretation.webp</code></li>';
    echo '<li><strong>تورم کیهانی</strong> (<code>cosmic-inflation</code>) &larr; <code>cosmic-inflation.webp</code></li>';
    echo '<li><strong>تابش زمینه کیهانی</strong> (<code>cosmic-microwave-background</code>) &larr; <code>cosmic-microwave-background.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_4');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 04', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b04_execute() {
    ...
    $msg = 'عملیات بسته ' . 4 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
