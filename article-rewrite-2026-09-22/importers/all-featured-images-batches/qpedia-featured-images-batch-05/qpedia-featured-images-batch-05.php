<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 05 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 05 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-05
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b05_admin_menu');
function qp_fimg_b05_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 05',
        'تصاویر شاخص بسته 05',
        'manage_options',
        'qpedia-featured-images-batch-05',
        'qp_fimg_b05_render_page'
    );
}

function qp_fimg_b05_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 05</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_5');
        $result = qp_fimg_b05_execute();
    ...
    echo '<li><strong>الکترونیک سرد درون یخچال برودتی</strong> (<code>cryogenic-electronics</code>) &larr; <code>cryogenic-electronics.webp</code></li>';
    echo '<li><strong>انرژی تاریک کوانتومی</strong> (<code>dark-energy-quantum</code>) &larr; <code>dark-energy-quantum.webp</code></li>';
    echo '<li><strong>واهمدوسی کوانتومی</strong> (<code>decoherence</code>) &larr; <code>decoherence.webp</code></li>';
    ...
    echo '<li><strong>مدار الگوریتم دویچ–جوزا</strong> (<code>deutsch-jozsa-algorithm</code>) &larr; <code>deutsch-jozsa-algorithm.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_5');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 05', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b05_execute() {
    ...
    $msg = 'عملیات بسته ' . 5 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
