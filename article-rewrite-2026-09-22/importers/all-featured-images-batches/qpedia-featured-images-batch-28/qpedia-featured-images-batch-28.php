<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 28 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 28 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-28
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b28_admin_menu');
function qp_fimg_b28_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 28',
        'تصاویر شاخص بسته 28',
        'manage_options',
        'qpedia-featured-images-batch-28',
        'qp_fimg_b28_render_page'
    );
}

function qp_fimg_b28_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 28</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_28');
        $result = qp_fimg_b28_execute();
    ...
    echo '<li><strong>تکینگی کوانتومی در فضا–زمان</strong> (<code>quantum-singularity</code>) &larr; <code>quantum-singularity.webp</code></li>';
    echo '<li><strong>سولیپسیسم کوانتومی و مسئله ناظر</strong> (<code>quantum-solipsism</code>) &larr; <code>quantum-solipsism.webp</code></li>';
    echo '<li><strong>اسپین؛ چرخشی که چرخش نیست</strong> (<code>quantum-spin</code>) &larr; <code>quantum-spin.webp</code></li>';
    echo '<li><strong>حالت کوانتومی چیست؟</strong> (<code>quantum-state</code>) &larr; <code>quantum-state.webp</code></li>';
    echo '<li><strong>برهم‌نهی کوانتومی</strong> (<code>quantum-superposition</code>) &larr; <code>quantum-superposition.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_28');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 28', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b28_execute() {
    ...
    $msg = 'عملیات بسته ' . 28 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
