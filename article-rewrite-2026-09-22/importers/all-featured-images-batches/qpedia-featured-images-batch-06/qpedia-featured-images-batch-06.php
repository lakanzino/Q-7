<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 06 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 06 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-06
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b06_admin_menu');
function qp_fimg_b06_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 06',
        'تصاویر شاخص بسته 06',
        'manage_options',
        'qpedia-featured-images-batch-06',
        'qp_fimg_b06_render_page'
    );
}

function qp_fimg_b06_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 06</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_6');
        $result = qp_fimg_b06_execute();
    ...
    echo '<li><strong>توزیع کلید کوانتومی مستقل از دستگاه</strong> (<code>device-independent-qkd</code>) &larr; <code>device-independent-qkd.webp</code></li>';
    echo '<li><strong>یخچال رقیق‌سازی و مراحل دمایی آن</strong> (<code>dilution-refrigerator</code>) &larr; <code>dilution-refrigerator.webp</code></li>';
    echo '<li><strong>گاز دوقطبی و آرایش ذرات قطبی</strong> (<code>dipolar-gas</code>) &larr; <code>dipolar-gas.webp</code></li>';
    echo '<li><strong>نیمه‌فلز دیراک</strong> (<code>dirac-semimetal</code>) &larr; <code>dirac-semimetal.webp</code></li>';
    echo '<li><strong>کوانتوم و خدا</strong> (<code>does-quantum-prove-god</code>) &larr; <code>does-quantum-prove-god.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_6');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 06', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b06_execute() {
    ...
    $msg = 'عملیات بسته ' . 6 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
