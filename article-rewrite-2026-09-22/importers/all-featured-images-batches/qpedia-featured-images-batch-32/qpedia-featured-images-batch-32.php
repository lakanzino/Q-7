<?php
/**
 * Plugin Name: Qpedia Featured Images - Batch 32 (5 Images)
 * Description: افزونه تنظیم و جایگزینی تصاویر شاخص بهینه‌شده WebP با نشان استاندارد qpedia.ir (بسته شماره 32 شامل 5 مقاله) به همراه متادیتا و Alt Text سئو در Qpedia.
 * Version: 1.0.0
 * Author: Qpedia Team
 * Text Domain: qpedia-featured-images-batch-32
 */

defined('ABSPATH') || exit;

add_action('admin_menu', 'qp_fimg_b32_admin_menu');
function qp_fimg_b32_admin_menu() {
    add_management_page(
        'تصاویر شاخص کوانتوم - بسته 32',
        'تصاویر شاخص بسته 32',
        'manage_options',
        'qpedia-featured-images-batch-32',
        'qp_fimg_b32_render_page'
    );
}

function qp_fimg_b32_render_page() {
    if (!current_user_can('manage_options')) {
        return;
    }

    echo '<div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif;line-height:1.7;">';
    echo '<h1>🖼️ تنظیم و جایگزینی تصاویر شاخص WebP - بسته 32</h1>';

    if (isset($_POST['qp_run_attach'])) {
        check_admin_referer('qp_attach_nonce_32');
        $result = qp_fimg_b32_execute();
    ...
    echo '<li><strong>تداخل‌سنج کوانتومی ابررسانا اسکویید</strong> (<code>squid</code>) &larr; <code>squid.webp</code></li>';
    echo '<li><strong>ابر تفکیک کوانتومی</strong> (<code>super-resolution-quantum</code>) &larr; <code>super-resolution-quantum.webp</code></li>';
    echo '<li><strong>پارادوکس سه جعبه</strong> (<code>three-box-paradox</code>) &larr; <code>three-box-paradox.webp</code></li>';
    echo '<li><strong>ساختار عایق توپولوژیک</strong> (<code>topological-insulator</code>) &larr; <code>topological-insulator.webp</code></li>';
    echo '<li><strong>توزیع کلید کوانتومی میدان دوقلو</strong> (<code>twin-field-qkd</code>) &larr; <code>twin-field-qkd.webp</code></li>';
    echo '</ol>';

    echo '<form method="post" style="margin-top:20px;">';
    wp_nonce_field('qp_attach_nonce_32');
    submit_button('شروع اتصال و جایگزینی تصاویر شاخص بسته 32', 'primary', 'qp_run_attach');
    echo '</form>';
    echo '</div></div>';
}

function qp_fimg_b32_execute() {
    ...
    $msg = 'عملیات بسته ' . 32 . ' به پایان رسید: ' . $count_ok . ' تصویر شاخص با موفقیت متصل گردید.';
