<?php
/**
 * Plugin Name: Qpedia One-Time Importer 066–075
 * Description: جایگزینی کنترل‌شده مقاله‌های ۶۶ تا ۷۵ Qpedia و ثبت متادیتای Rank Math؛ فقط یک بار قابل اجرا است.
 * Version: 1.0.0
 * Author: Qpedia Editorial
 */
if (!defined('ABSPATH')) exit;

const QP6675_OPTION = 'qpedia_importer_066_075_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 066–075', 'درون‌ریز Qpedia 066–075', 'manage_options', 'qpedia-importer-066-075', 'qp6675_render_page');
});

function qp6675_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP6675_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۶۶ تا ۷۵</h1>';
    if ($done) {
        echo '<div class="notice notice-success"><p>این بسته قبلاً در '.esc_html($done).' اجرا شده است و دوباره اجرا نمی‌شود.</p></div></div>';
        return;
    }
    if (isset($_POST['qp6675_run'])) {
        check_admin_referer('qp6675_run_import');
        $results = qp6675_run_import();
        $has_error = false;
        ...
        echo '</ul></div>';
        if (!$has_error) {
            update_option(QP6675_OPTION, current_time('mysql'), false);
            echo '<p><strong>هر ۱۰ مقاله به‌روزرسانی شدند. افزونه را اکنون غیرفعال و حذف کنید.</strong></p></div>';
            return;
        }
    }
    ...
    wp_nonce_field('qp6675_run_import');
    submit_button('به‌روزرسانی مقاله‌های ۶۶ تا ۷۵', 'primary', 'qp6675_run');
    echo '</form></div>';
}

function qp6675_run_import() {
    $path = plugin_dir_path(__FILE__).'articles.json';
    $items = json_decode(file_get_contents($path), true);
    if (!is_array($items) || count($items) !== 10) return array(array('ok'=>false, 'message'=>'خطا: فایل داده معتبر نیست.'));
    $results = array();
