<?php
/**
 * Plugin Name: Qpedia One-Time Importer 006–010
 * Description: جایگزینی کنترل‌شده مقاله‌های ششم تا دهم Qpedia و ثبت متادیتای Rank Math؛ فقط یک بار قابل اجرا است.
 * Version: 1.0.0
 * Author: Qpedia Editorial
 */
if (!defined('ABSPATH')) exit;

const QP5_OPTION = 'qpedia_importer_006_010_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 006–010', 'درون‌ریز Qpedia 006–010', 'manage_options', 'qpedia-importer-006-010', 'qp5_render_page');
});

function qp5_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP5_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۶ تا ۱۰</h1>';
    if ($done) {
    ...
    submit_button('به‌روزرسانی مقاله‌های ۶ تا ۱۰', 'primary', 'qp5_run');
    echo '</form></div>';
}
