<?php
/**
 * Plugin Name: Qpedia One-Time Importer 056–065
 * Description: جایگزینی کنترل‌شده مقاله‌های ۵۶ تا ۶۵ Qpedia و ثبت متادیتای Rank Math؛ فقط یک بار قابل اجرا است.
 * Version: 1.0.0
 * Author: Qpedia Editorial
 */
if (!defined('ABSPATH')) exit;

const QP10_OPTION = 'qpedia_importer_056_065_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 056–065', 'درون‌ریز Qpedia 056–065', 'manage_options', 'qpedia-importer-056-065', 'qp10_render_page');
});

function qp10_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP10_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۵۶ تا ۶۵</h1>';
    if ($done) {
        echo '<div class="notice notice-success"><p>این بسته قبلاً در '.esc_html($done).' اجرا شده است و دوباره اجرا نمی‌شود.</p></div></div>';
        return;
    }
    if (isset($_POST['qp10_run'])) {
        check_admin_referer('qp10_run_import');
        $results = qp10_run_import();
        $has_error = false;
        ...
        echo '</ul></div>';
        if (!$has_error) {
            update_option(QP10_OPTION, current_time('mysql'), false);
    ...
    wp_nonce_field('qp10_run_import');
    submit_button('به‌روزرسانی مقاله‌های ۵۶ تا ۶۵', 'primary', 'qp10_run');
    echo '</form></div>';
}

function qp10_run_import() {
