<?php
/**
 * Plugin Name: Qpedia One-Time Importer 076–090
 * Description: جایگزینی کنترل‌شده مقاله‌های ۷۶ تا ۹۰ Qpedia و ثبت متادیتای Rank Math؛ فقط یک بار قابل اجرا است.
 * Version: 1.0.0
 * Author: Qpedia Editorial
 */
if (!defined('ABSPATH')) exit;

const QP157690_OPTION = 'qpedia_importer_076_090_completed';

add_action('admin_menu', function () {
    add_management_page('درون‌ریز Qpedia 076–090', 'درون‌ریز Qpedia 076–090', 'manage_options', 'qpedia-importer-076-090', 'qp157690_render_page');
});

function qp157690_render_page() {
    if (!current_user_can('manage_options')) return;
    $done = get_option(QP157690_OPTION);
    echo '<div class="wrap" dir="rtl"><h1>درون‌ریز یک‌بارمصرف مقاله‌های ۷۶ تا ۹۰</h1>';
    if ($done) {
        echo '<div class="notice notice-success"><p>این بسته قبلاً در '.esc_html($done).' اجرا شده است و دوباره اجرا نمی‌شود.</p></div></div>';
        return;
    }
    if (isset($_POST['qp157690_run'])) {
        check_admin_referer('qp157690_run_import');
        $results = qp157690_run_import();
        $has_error = false;
        ...
        echo '</ul></div>';
        if (!$has_error) {
            update_option(QP157690_OPTION, current_time('mysql'), false);
            echo '<p><strong>هر ۱۵ مقاله به‌روزرسانی شدند. افزونه را اکنون غیرفعال و حذف کنید.</strong></p></div>';
            return;
        }
    }
    ...
    wp_nonce_field('qp157690_run_import');
    submit_button('به‌روزرسانی مقاله‌های ۷۶ تا ۹۰', 'primary', 'qp157690_run');
    echo '</form></div>';
}

function qp157690_run_import() {
    $path = plugin_dir_path(__FILE__).'articles.json';
    $items = json_decode(file_get_contents($path), true);
    if (!is_array($items) || count($items) !== 15) return array(array('ok'=>false, 'message'=>'خطا: فایل داده معتبر نیست.'));
    $results = array();
